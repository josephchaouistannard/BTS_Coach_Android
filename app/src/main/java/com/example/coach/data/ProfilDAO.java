...
public class ProfilDAO  extends SQLiteOpenHelper {
    ...
    public ProfilDAO(@Nullable Context context) {
    ...
    public void onCreate(SQLiteDatabase db) {
        String createTable = "CREATE TABLE " + TABLE_PROFIL + " (" +
                COL_DATEMESURE + " INTEGER PRIMARY KEY, " + // stocké en timestamp
                COL_POIDS + " INTEGER, " +
                COL_TAILLE + " INTEGER, " +
                COL_AGE + " INTEGER, " +
                COL_SEXE + " INTEGER)";
    ...
    public Profil getLastProfil() {
        ...
        if (cursor.moveToFirst()) {
            ...
            long dateMesure = cursor.getLong(cursor.getColumnIndexOrThrow(COL_DATEMESURE));
